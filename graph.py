...
def distance_scipy_spatial(z, k=4, metric='euclidean'):
    ...
    d = scipy.spatial.distance.pdist(z, metric)  #调用函数计算输入矩阵z中所有样本对之间的距离，返回一个一维数组，已压缩形式存储，如果有n个样本，则结果长度为n*（n-1）/2
    d = scipy.spatial.distance.squareform(d)  #将计算结果转变成方阵形式
    # k-NN graph.
    idx = np.argsort(d)[:, 1:k+1]  #将每行元素从小到大排序，并返回索引，从第二个元素开始，因为索引0对应于自身与自身的距离
    d.sort()
    d = d[:, 1:k+1]
    return d, idx
...
def distance_sklearn_metrics(z, k=4, metric='euclidean'):
    ...
    d = sklearn.metrics.pairwise.pairwise_distances(
            z, metric=metric, n_jobs=-2)  #n_job表示并行计算，能够提高计算速度
    # k-NN graph.
