...
import sys, os
# sys.path.insert(0, '..')
import graph, coarsening, read_data, model_test
...
# flags = tf.app.flags
# FLAGS = flags.FLAGS
parser = argparse.ArgumentParser(description='Your program description') #创建一个解析器对象
...
parser.add_argument('--number_edges',type=int,default=8,help='Graph:minimum number of edges per vertex')
...
class model_perf(object):
    ...
    def save(self):
        ...
        if os.path.exists('_results.csv'):
            ...
            old = pd.read_csv('_results.csv', header=None)
            ...
            r.to_csv('_results.csv', index=False, header=['method', 'test_acc',
    ...
    def fin_result(self, i_fold=None):
        for name in sorted(self.names):
        ...
        if i_fold == 4:
            ...
            r.to_csv('../../../data/output/' + '_fin_results', index=False, header=['method', 'test_auc'])
...
def train(method, view_com, n_views, k, m, n_epoch, batch_size, pairs, labels, coords, subj, data, i_fold):
    ...
    coo1, coo2, coo3 = coords.shape # coo2 is the roi dimension  #599*84*3,84是ROI
    ...
    if method == 'fnn':
        ...
        mp.test(model_test.siamese_fnn(L, **params), name, params,
    ...
    if method == 'gcn':
        ...
        params['brelu'] = 'b2relu'
        params['pool'] = 'apool1'
        mp.test(model_test.siamese_m_cgcnn(L, **params), name, params,
                        data, train_pairs, train_y, val_x, val_y, test_pairs, test_y)
    ...
    mp.fin_result(i_fold)
    # mp.fin_result(method_type + data_type + str_params + obj_params, i_fold)

# def run_program(method,data_type1,view_com,kfold,K,M,n_epoch,batch_size):
def run_program(method, view_com, kfold, K, M, n_epoch, batch_size):
    ...
    # args = parser.parse_args()
    # print ('---------------------------------------')
    # data_type = [args.data_type1, args.data_type2, args.data_type3]
    # data_type=[args.data_type1]
    # data_type = data_type1
    # n_views = len(data_type)
    n_views = 1
    # See function train for all possible parameter and there definition.
    # data, subj, coords, pairs, labels = read_data.load_data(data_type=data_type, kfold=args.kfold)
    data, subj, coords, pairs, labels = read_data.load_data(kfold=kfold)
    # data_type = args.data_type1 + '+' + args.data_type2  + '+' + args.data_type3
    # data_type=args.data_type1
    # print (data.shape)
    if kfold == 'True':
...
if __name__ == '__main__':
    # "gcn" $view "max" "True" 30 128 20 32
    run_program(method="2gcn",view_com="max",kfold="True",K=30,M=128,n_epoch=20,batch_size=32)